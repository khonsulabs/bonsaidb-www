...
Back in the early 2000s, I ran several distros at home and at work, and back
then, I don't remember `/tmp` being special. It's not special on the Mac, which
is one factor in why Nebari's benchmarks exhibited a major change on the Mac
while no change on Linux.
...
a one-size-fits-all solution is almost never the solution.

The benefits of Nebari come down to it being tailor-fit to BonsaiDb's needs.
With the new storage format that leverages Nebari's ability to embed custom
stats within its B+Tree, it meant I could build and query optimized map-reduce
views in a very efficient manner. I have yet to see another low-level database
written in Rust that enables embedding custom information inside of the B+Tree
structure to enable these capabilities.

Because a tailor-fit solution is so attractive, I wanted to see if I could
figure out a way to speed Nebari up.

## {{ anchor(text = "Why is Nebari slow?" )}}

To answer that question, I needed to fix my usage of `tempfile` in the Nebari
benchmark to create files in the current directory rather than the temporary
directory. After doing that, Nebari still competed with
...
return in less than 1 millisecond on my machine. Ever. End of story. Here's the
output if the transactional insert benchmark measuring the time it takes to do
an ACID-complaint insert of 1KB of data to a new key:
...
tempted to infer that since the max time reported for Sled by Criterion is
39.4μs as I was. However, Criterion uses sampling-based statistics, which means
that it doesn't look at individual iteration times but rather iteration times
for a set number of iterations.

By logging out each individual iteration time, I can see that there are
individual iterations that *do* take as long as an `fdatasync` call. It's the
reporting resolution that Criterion provides that prevents these numbers from
showing up. The takeaway is still important: usig `sync_file_range`, Sled is
able to make the average write's time take 38.9μs, even though occasionally
there will be write operations that are longer than 1ms due to the need for an
`fdatasync`.
...
machine. The user hit Save in their program, and the program said it saved the
file, but upon powering back up after the power plug was pulled, the file is
missing or corrupt. The reason? The file may have only been partially written to
disk when the plug was pulled.
...
- On Mac, `fsync()` is available but does not provide the same guarantees as
  Linux. Instead, a call to `fcntl` with the `F_FULLFSYNC` option must be used
  to trigger a write to physical media.

Rust uses the proper APIs for each platform when calling `File::sync_all` or
`File::sync_data` to provide durable writes but doesn't provide APIs to invoke
the underlying APIs independently. Thankfully, the `libc` crate makes it easy to
call these APIs.
...
> into preallocated space, many filesystems also require calls into the block
...
aware][sled-issue] of this warning. One of the commentors points out that
RocksDB [has special code to disable using the API on `zfs`][rocksdb-zfs].
Pebble, which is a Go spinoff of RocksDB, takes the approach of [opting-in
`ext4`][pebble-ext4]. Both RocksDB and Pebble seem to still use
`fsync`/`fdatasync` at various locations to ensure durability.
...
post][justinj-blog] where he performed similar tests against `fsync`.

It turns out if you run this command with superuser permissions on a Linux
machine, it will immediately power off: `echo o > /proc/sysrq-trigger`. This
greatly simplified my testing setup.

I executed a VM using this command in a loop so that it automatically restarted itself:
...
- `sync_file_range.rs`: When initializing the data file, zeroes are manually
  written to the file.
- `sync_file_range_set_len.rs`: When initializing the data file,
  `File::set_len()` is called to extend the file, which is documented currently as:
...
Despite what the Rust documentation says, under the hood, it uses `ftruncate`,
...
subtle, but very important when considering the safety of `sync_file_range`.
From my testing, using `ftruncate` to fill pages with 0 will conflict with
`sync_file_range` on the first operation, but will likely succeed on all future
tests.
...
- `sync_file_range` is only safe to use on fully initialized pages. `ftruncate` to extend a
  file does not fully initialize those pages with zeroes.

## {{ anchor(text = "Does F_BARRIERFSYNC provide durable writes on Mac OS/iOS?" )}}
...
But, to confuse things further, it continues by saying:
...
Apple really dropped the ball here. Their own docs point out that `fsync()`
doesn't provide the same guarantees as other operating systems' implementations
and suggests using `F_FULLFSYNC`. However, they then proceed to point out that
even `F_FULLFSYNC` isn't enough to protect against sudden power loss.

According to all available documentation I can find: there is no way to execute
a truly ACID-compliant database on Mac OS. You can get close, but a power loss
could still result in a write being reported as successfully persisted being
gone after a power outage.

SQLite by default uses `F_BARRIERFSYNC` for all of its file synchronization on
Mac/iOS. Optionally, you can use a `#pragma` to enable usage of `F_FULLFSYNC`.

My personal understanding for Mac OS, a viable solution to a 2-step syncing
process would be to use `F_BARRIERFSYNC` to ensure changed pages are persisted
between the two steps in the sync process. The final sync, however, must be
issued with `F_FULLFSYNC` to ensure maximum durability.

## {{ anchor(text = "Does Windows offer any partial-syncing APIs?" )}}

No. The only API avialable on Windows is [`FlushFileBuffers`][flushfilebuffers].
...
in performance to SQLite even post-sync changes. This is true, but it's not an
apples-to-apples comparison, and the difference is the primary reason BonsaiDb
slowed down.

SQLite has many approaches to persistence, so let's look at the journaled
version as it's simpler. When using a journal, SQLite creates a file that
contains information needed to undo the changes its about to make to the
database file. To ensure a consistent state that can be recovered after a power
outage at any point in this process, it must make at least two `fsync` calls.
...
This dual-sync nature with only a few collections being written to is what
caused the Commerce Benchmark to grind to a halt. Individual worker threads
would back up waiting for their turn to modify a collection.

Nebari's architecture was designed in October, and I spent countless hours
profiling and testing its performance. Due to the aforementioned issues, so many
of my performance assumptions were flat out wrong.
...
finally started feeling confidence in its reliability. Then, this series of
events hit me. On one hand, seeing that Nebari is still pretty fast after fixing
this bug should prove to me that I *can* write a fast database. On the other
hand, I'm so embarrassed I didn't notice these issues earlier, and it's
demoralizing to think of all the time spent building upon mistaken assumptions.
